    ...
    for (const testCase of SAVED_ADDRESS_CHECKOUT_CASES) {
        test(
            testCase.description,
            {
                tag: ['@checkout', '@saved'],
                annotation: [
                    { type: 'Shipping method', description: testCase.shippingMethod },
                    { type: 'Payment method', description: testCase.paymentMethod },
                    { type: 'Expected payment info', description: testCase.expectedPaymentInfo },
                ],
            },
            async ({ checkoutSteps }) => {
...
test.describe('Checkout - New Address', () => {
    test.beforeEach(async ({ sharedSteps, logInSteps, categorySteps, productSteps, cartSteps }) => {
        await sharedSteps.navigateToLoginPage();
        await logInSteps.loginUser(CHECKOUT_USER);
    ...
    for (const testCase of NEW_ADDRESS_CHECKOUT_CASES) {
        test(
            testCase.description,
            {
                tag: ['@checkout', '@new'],
